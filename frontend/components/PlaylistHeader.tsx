...
export default function PlaylistHeader({ playlist, tracks, onPlay, onShuffle, onEdit, onShare }: Props) {
  ...
  return (
    <div className="mb-5">
      {/* Spotify-style banner: full-bleed, short, name overlaid, fading into the page */}
      <div className="relative -mx-4 h-52 sm:h-60 md:-mx-8">
        {cover ? (
        ...
        <div className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-app via-app/80 to-transparent" />
        <h1 className="absolute right-4 bottom-2 left-4 line-clamp-2 text-3xl font-extrabold tracking-tight text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)] sm:text-4xl md:left-8">
          {playlist.name}
        </h1>
      </div>
